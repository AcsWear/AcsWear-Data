...
local USERNAME_GH = "AcsWear"
local REPOSITORY_GH = "AcsWear-Data"
                    ...
                    if success and data then
                        ...
                        if latestVersion ~= "" then
                            ...
                            if comparison == -1 then
                                ...
                                sampAddChatMessage(
                                    "{00deff}[AcsWear] {FFFFFF}Äîñòóïíî îáíîâëåíèå {00deff}" ..
                                        current ..
                                            " > " ..
                                                latestVersion ..
...
local savedPresets = {ini.presets.p1, ini.presets.p2, ini.presets.p3}
...
local default_items_data = {
    --slot 1 (íèìá)
    {
		...
		{u8'Ìàðêåð Dead Eblan', 5013, 2 , 0.4289990067482, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, false, false},
...
local popular_skins = {
    ...
    {id = 9749, name = u8("Ïðèçðà÷íîå ëèöî")},
...
local armour_color_list_data = {
    {u8 "Ñòàíäàðòíûé", -1},
    {u8 "ïîäïèøèñü íà íàø òã", -16776961}, -- õç íàõóÿ òóò ýòî íî ïóñòü áóäåò
    {u8 "Êðàñíûé", -16711936},
    {u8 "Çåëåíûé", -65536},
    {u8 "Ñèíèé", -65281},
    {u8 "Ðîçîâûé", -16744193},
...
local ad = {
    ...
    mainWindowSize = 300,
    vertical_offset = 5,
    text_header = "Telegram",
    link = "https://t.me/rodinachecker_bot?start=411104ECKF",
    IMAGE = nil,
    image_path = getWorkingDirectory() .. "\\resource\\AcsWear\\bannerads.png",
    image_link = "https://i.ibb.co/bRHxCfPj/rodinachecker.png"
}
...
function main()
    ...
    sampAddChatMessage("{00deff} {FFFFFF}Àâòîð: {00deff}Luffich", 0xFFFFFFFF)
    sampAddChatMessage("{00deff} {FFFFFF}/acsw {00deff}- {FFFFFF}Îòêðûòü ìåíþ", 0xFFFFFFFF)
...
function imgui.OnDrawFrame()
    ...
    if window.v then
        ...
        elseif menu_selected.v == 7 then
            ...
            if changeskin.v then
                ...
                imgui.SetCursorPosX(75)
                if imgui.Button(u8 "Ïîïóëÿðíûå ñêèíû", imgui.ImVec2(150, 24)) then
        ...
        elseif menu_selected.v == 8 then
            ...
            elseif updateAvailable then
                ...
                if imgui.Button(u8 "Îáíîâèòü ñåé÷àñ", imgui.ImVec2(270, 30)) then
                    performUpdate()
                end
            else
                imgui.TextColored(imgui.ImVec4(0.0, 1.0, 0.0, 1.00), u8 "Ñòàòóñ: Àêòóàëüíàÿ âåðñèÿ")
                imgui.Text(u8 "Âåðñèÿ: " .. currentVersion)
            end

            imgui.Spacing()

            if imgui.Button(u8 "Ïðîâåðèòü îáíîâëåíèÿ", imgui.ImVec2(270, 23)) then
                checkForUpdates(true)
            end

            if imgui.IsItemHovered() then
                imgui.BeginTooltip()
                imgui.Text(u8 "Ïðîâåðèòü íàëè÷èå îáíîâëåíèé íà GitHub")
                imgui.EndTooltip()
            end
        ...
        if menu_selected.v < 7 then
            ...
            if imgui.InvisibleButton("button1", imgui.ImVec2(110, 88)) then
                os.execute('explorer "https://acswear.github.io/AcsWear"')
            end
            ...
            local text1 = u8 "Ñïèñîê"
            local text2 = u8 "Ïðåäìåòîâ"
        ...
        imgui.Text(u8 "Àâòîð:")
        imgui.SameLine()
        imgui.TextColored(imgui.ImVec4(0.0, 0.75, 1.0, 1.00), "Luffich")

        if imgui.IsItemClicked(0) then
            imgui.OpenPopup(u8 "Àâòîð: Luffich")
        end
        if imgui.IsItemHovered() then
            imgui.BeginTooltip()
        ...
        if
         ...
         then
            ...
            if imgui.Button(u8 "VK", imgui.ImVec2(80, 40)) then
                os.execute('explorer "https://vk.com/id565176942"')
            end
            imgui.SameLine()
            if imgui.Button(u8 "Telegram", imgui.ImVec2(80, 40)) then
                os.execute('explorer "https://t.me/rodina_script"')
            end
            imgui.SameLine()
            if imgui.Button(u8 "Òåìà BH", imgui.ImVec2(80, 40)) then
                os.execute('explorer "https://www.blast.hk/threads/242357/"')
            end
            ...
            imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1.0), u8 "Ïî âñåì âîïðîñàì â Telegram.")
            imgui.Spacing()
    ...
    if show_popular_skins.v then
        ...
        imgui.Begin(u8("Ïîïóëÿðíûå ñêèíû"), show_popular_skins, imgui.WindowFlags.NoResize)

        imgui.Text(u8("Âûáåðèòå ïîïóëÿðíûé ñêèí:"))
...
function save()
    ...
    ini.presets.p1, ini.presets.p2, ini.presets.p3 = savedPresets[1], savedPresets[2], savedPresets[3]
